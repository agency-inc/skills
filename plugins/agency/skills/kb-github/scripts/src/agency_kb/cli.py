...
def preview(
    ...
    """Preview which source files match each article's glob patterns."""
    ...
    exported_docs = [
        ExportedOutlineDocument(
            ...
            globs=a.globs,
            outline_markdown="\n".join(f"- {t}" for t in a.topics),
        )
        for a in parsed_outline.articles
    ]
    exported_outline = ExportedOutline(
    ...
    analysis = run_analyze(outline=exported_outline, repo_root=resolved_root)

    for doc in analysis.documents:
        table = Table(title=f"{doc.title} ({doc.document_path})")
        table.add_column("Type", style="cyan", width=12)
        table.add_column("Path")

        for f in doc.relevant_files:
            table.add_row("relevant", f)
        for f in doc.candidate_files:
            table.add_row("candidate", f"{f.path} ({f.reason})")
        for f in doc.missing_files:
            table.add_row("[red]missing[/red]", f)
        for g in doc.unmatched_globs:
            table.add_row("[yellow]no match[/yellow]", g)

        if not doc.relevant_files and not doc.candidate_files:
            table.add_row("[yellow]none[/yellow]", "No files matched any glob pattern")

        rich.print(table)
        rich.print()

    total_relevant = sum(len(d.relevant_files) for d in analysis.documents)
    total_candidates = sum(len(d.candidate_files) for d in analysis.documents)
    rich.print(
        f"[green]{total_relevant} relevant files[/green], "
        f"[yellow]{total_candidates} candidates[/yellow] "
        f"across {len(analysis.documents)} articles"
    )
